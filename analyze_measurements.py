...
def get_measurement_stats(data, file_path):
    ...
    print(f"  Valid points: {stats['total_points'] - stats['nan_count']} out of {stats['total_points']} ({100*(1-stats['nan_count']/stats['total_points']):.2f}%)")
    ...
    print(f"  {continuous_axis}-axis: {len(points_continuous)} points from {np.min(points_continuous):.1f} to {np.max(points_continuous):.1f} mm")
    print(f"  {discrete_axis}-axis: {len(points_discrete)} points from {np.min(points_discrete):.1f} to {np.max(points_discrete):.1f} mm")
    
    if 'frequency' in data:
        print(f"  Frequency: {data['frequency']/1e9:.6f} GHz (wavelength: {299792458/(data['frequency']/1e9)/1e6:.6f} mm)")
...
def plot_measurement(data, file_path, in_log=False, method='contourf', output_dir='measurement_plots'):
    ...
    if method == 'contourf':
        contour = plt.contourf(cc, dd, results_to_plot, cmap='jet')
    elif method == 'imshow':
        plt.imshow(results_to_plot, cmap='jet', aspect='auto', 
                   extent=[np.min(points_continuous), np.max(points_continuous), 
                           np.min(points_discrete), np.max(points_discrete)])
    elif method == 'pcolor':
...
def plot_3d_slice_positions(measurement_files, output_dir='measurement_plots'):
    ...
    for i, file_path in enumerate(measurement_files):
        ...
        if filename.startswith('x'):
            ...
            y_vals = data['points_discrete'] if isinstance(data['points_discrete'], np.ndarray) else np.array(data['points_discrete'])
            ...
            ax.scatter(x_grid[::stride, ::stride], 
                      y_grid[::stride, ::stride], 
                      z_grid[::stride, ::stride], 
                      color=colors[i % len(colors)], alpha=0.5, s=10)
            ...
            z_vals = data['points_continuous']
            x_vals = data['points_discrete'] if isinstance(data['points_discrete'], np.ndarray) else np.array(data['points_discrete'])
...
if __name__ == "__main__":
    ...
    measurement_files = [os.path.join(measurement_dir, f) for f in os.listdir(measurement_dir) if f.endswith('.pickle')]
    ...
    # Create the table
    column_labels = ['File', 'Min (V/m)', 'Max (V/m)', 'Mean (V/m)', 'Median (V/m)', 'Std Dev (V/m)', 'Valid Data']
