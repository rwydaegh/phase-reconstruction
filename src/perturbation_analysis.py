...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Analyze perturbation effect on field reconstruction")
    
    # Perturbation settings
    parser.add_argument('--perturbation-factors', type=str, default="0,0.001,0.01,0.05,0.1",
                        help='Comma-separated list of perturbation factors to test')
    parser.add_argument('--wall-points', type=int, default=10, 
                        help='Points per wall edge (default: 10)')
    ...
    parser.add_argument('--num-sources', type=int, default=50, 
                        help='Number of sources (default: 50)')
    parser.add_argument('--output-dir', type=str, default='perturbation_analysis', 
...
def create_measurement_plane(config):
    ...
    x = np.linspace(config.room_size/2 - config.plane_size/2, 
                    config.room_size/2 + config.plane_size/2, resolution)
    y = np.linspace(config.room_size/2 - config.plane_size/2, 
                    config.room_size/2 + config.plane_size/2, resolution)
    X, Y = np.meshgrid(x, y)
...
def create_focused_perturbation_plots(all_results, perturbation_factors, args):
    """Create focused plots showing the relationship between perturbation amount and RMSE/correlation"""
    ...
    plt.plot(perturbation_factors, rmse_self, 's-', color='blue', linewidth=2, 
             label='Self-reconstruction with perturbed geometry')
    plt.plot(perturbation_factors, rmse_orig, '^-', color='green', linewidth=2, 
    ...
    plt.plot(perturbation_factors, corr_real, 'o-', color='red', linewidth=2, 
             label='Realistic: Perturbed geometry with true currents')
    plt.plot(perturbation_factors, corr_overfit, 'x-', color='purple', linewidth=2, 
             label='Overfitting: Perturbed geometry with optimized currents')
    plt.plot(perturbation_factors, corr_self, 's-', color='blue', linewidth=2, 
             label='Self-reconstruction with perturbed geometry')
    plt.plot(perturbation_factors, corr_orig, '^-', color='green', linewidth=2, 
             label='Original reconstruction')
    ...
    ax2.plot(perturbation_factors, corr_real, 'o-', color='red', linewidth=2, 
            label='Realistic (fixed currents)')
    ax2.plot(perturbation_factors, corr_overfit, 'x-', color='purple', linewidth=2, 
            label='Overfitting (optimized currents)')
    ...
    table = ax.table(
        cellText=[[f"{row[0]:.6f}" if i == 0 else f"{row[i]:.6f}" for i in range(len(row))] for row in table_data],
...
def main():
    ...
    for factor in perturbation_factors:
        ...
        for i, idx in enumerate(original_source_indices):
        ...
        original_true_field_2d = np.abs(original_true_field).reshape(config.resolution, config.resolution)
        perturbed_true_field_2d = np.abs(perturbed_true_field).reshape(config.resolution, config.resolution)
        ...
        original_recon_field_2d = np.abs(original_recon_field).reshape(config.resolution, config.resolution)
        ...
        original_vs_original_metrics = {
            ...
            "correlation": normalized_correlation(np.abs(original_true_field), np.abs(original_recon_field))
        }
        print(f"Original recon vs Original true - RMSE: {original_vs_original_metrics['rmse']:.6f}, Correlation: {original_vs_original_metrics['correlation']:.6f}")
        ...
        # Store reshaped field data
        perturbed_forward_field_2d = np.abs(forward_field).reshape(config.resolution, config.resolution)
        ...
        perturbed_forward_metrics = {
            ...
            "correlation": normalized_correlation(np.abs(original_true_field), np.abs(forward_field))
        }
        print(f"Perturbed model (true currents) vs Original true - RMSE: {perturbed_forward_metrics['rmse']:.6f}, Correlation: {perturbed_forward_metrics['correlation']:.6f}")
        ...
        # Store reshaped field data
        perturbed_overfitting_field_2d = np.abs(perturbed_overfitting_field).reshape(config.resolution, config.resolution)
        
        # Evaluate perturbed reconstruction against original true field
        perturbed_overfitting_metrics = {
            "rmse": normalized_rmse(np.abs(original_true_field), np.abs(perturbed_overfitting_field)),
            "correlation": normalized_correlation(np.abs(original_true_field), np.abs(perturbed_overfitting_field))
        }
        print(f"Perturbed model (optimized currents) vs Original true - RMSE: {perturbed_overfitting_metrics['rmse']:.6f}, Correlation: {perturbed_overfitting_metrics['correlation']:.6f}")
        ...
        perturbed_vs_perturbed_metrics = {
            "rmse": normalized_rmse(np.abs(perturbed_true_field), np.abs(perturbed_self_recon_field)),
            "correlation": normalized_correlation(np.abs(perturbed_true_field), np.abs(perturbed_self_recon_field))
        }
        print(f"Perturbed recon vs Perturbed true - RMSE: {perturbed_vs_perturbed_metrics['rmse']:.6f}, Correlation: {perturbed_vs_perturbed_metrics['correlation']:.6f}")
