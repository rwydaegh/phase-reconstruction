...
def generate_original_currents(
    points_true: np.ndarray, cfg: DictConfig, use_vector_model: bool
) -> np.ndarray:
    """
    ...
    source_indices = random.sample(
        range(N_c), num_sources_to_activate
...
def get_source_pointcloud(cfg: DictConfig, global_cfg: DictConfig, base_dir: str = ".") -> tuple:
    ...
    if cfg.get("use_source_file", False):
        ...
        if data.shape[1] == 13:  # x,y,z, dist, nx,ny,nz, t1x,t1y,t1z, t2x,t2y,t2z
            ...
            logger.info(
                f"  Calculated tangents. t1: {tangents1_loaded.shape}, t2: {tangents2_loaded.shape}"
            )
            # Optional: Overwrite file with 13 columns? For now, just use calculated tangents.
        elif data.shape[1] >= 3:  # Assume at least x,y,z
            logger.warning(
                f"  Detected {data.shape[1]} columns. Using first 3 as points. Tangents will be None."
            )
            points_loaded = data[:, :3]
            ...
            raise ValueError(
                f"Loaded point cloud data has unexpected shape: {data.shape}. Need at least 3 columns."
            )
        ...
        gen_cfg = DictConfig(
            gen_cfg_dict
        )  # Create a temporary DictConfig if needed by the function
    ...
    # --- Apply Configured Translation ---
    # Apply translation *before* filtering/downsampling
    translation = np.array(cfg.get("translation", [0.0, 0.0, 0.0]))
    if np.any(translation != 0):
        logger.info(f"  Applying configured translation to source points: {translation}")
        points_true += translation
        # Tangents are direction vectors, so they don't change with translation
    else:
        logger.info("  No source point cloud translation configured.")
    ...
    if downsample_factor > 1 and points_true is not None:
        ...
        logger.info(
            f"  Downsampled point cloud from {original_count} to {len(points_true)} (factor: {downsample_factor})"
        )
    ...
    if cfg.get("perturb_points", False):
        ...
        logger.info(
            f"  Perturbing points for reconstruction geometry (factor: {perturbation_factor})..."
        )
        if perturbation_factor > 0:
            ...
            scaled_perturbations = (
                random_perturbations * perturbation_factor * distances[:, np.newaxis]
            )
            # Apply perturbation
    ...
    logger.info(
        f"Source handling complete. True points: {points_true.shape}, Perturbed points: {points_perturbed.shape}, Currents: {original_currents.shape}"
    )
