    ...
    for i, plane in enumerate(test_planes):
        ...
        if plane['is_real_plane']:
            ...
            if ground_truth_magnitude is None:
                 logger.error(f"    Cannot evaluate real plane '{plane_name}': 'measured_magnitude' missing.")
                 continue
        else:
            ...
            try:
                H_true_sim_plane = create_channel_matrix(
                    ...
                    measurement_direction=np.array(global_params.measurement_direction) if global_params.use_vector_model else None,
                )
                # Ensure current shape matches H_true_sim_plane columns
                expected_true_coeffs_len = points_true.shape[0] * (2 if global_params.use_vector_model else 1)
                if H_true_sim_plane.shape[1] != expected_true_coeffs_len:
                     raise ValueError(f"H_true columns ({H_true_sim_plane.shape[1]}) != expected from points_true ({expected_true_coeffs_len})")
                if original_currents.shape[0] != expected_true_coeffs_len:
                     raise ValueError(f"original_currents shape ({original_currents.shape[0]}) != expected from points_true ({expected_true_coeffs_len})")

                true_field_sim_plane = H_true_sim_plane @ original_currents
                ground_truth_magnitude = np.abs(true_field_sim_plane)
                logger.info(f"    Calculated ground truth magnitude shape: {ground_truth_magnitude.shape}")
            except Exception as e:
                logger.error(f"    Failed to calculate ground truth for simulated plane '{plane_name}': {e}", exc_info=True)
                continue
        ...
        try:
            ...
            if H_test_plane.shape[1] != final_coefficients.shape[0]:
                 raise ValueError(f"H_test_plane columns ({H_test_plane.shape[1]}) != final_coefficients length ({final_coefficients.shape[0]})")

        except Exception as e:
            logger.error(f"    Failed to create H matrix for test plane '{plane_name}': {e}", exc_info=True)
            continue
        ...
        if ground_truth_magnitude.shape != reconstructed_magnitude_test.shape:
             logger.error(f"    Shape mismatch between ground truth ({ground_truth_magnitude.shape}) "
        ...
        try:
            ...
            evaluation_results[plane_name] = {
                ...
                'original_data_shape': plane.get('original_data_shape') # Store shape for reshaping later
            }
        except Exception as e:
            logger.error(f"    Failed to calculate metrics for plane '{plane_name}': {e}", exc_info=True)
