...
def initialize_field_values(measured_magnitude):
    ...
    field_values = measured_magnitude * np.exp(1j * np.random.uniform(0, 2*np.pi, measured_magnitude.shape))
    return field_values
...
def apply_magnitude_constraint(simulated_field, measured_magnitude, skip_magnitude_constraint=False):
...
def update_memory_solutions(best_coefficients, best_error, memory_solutions, memory_errors, memory_ensemble_size):
...
def apply_momentum_perturbation(field_values, previous_momentum=None, 
                              perturbation_intensity=0.2, momentum_factor=0.8,
...
def perform_intelligent_restart(measured_magnitude, channel_matrix, memory_solutions, memory_errors, 
                               temperature, temperature_initial, memory_contribution, verbose=False):
    ...
    if memory_solutions and np.random.rand() < 0.9:  # 90% chance to use memory
        ...
        ensemble_magnitude = np.abs(ensemble_field)
        ...
        noise = np.random.normal(0, noise_level, field_values.shape) + 1j * np.random.normal(0, noise_level, field_values.shape)
    ...
    if verbose:
        logger.info(f"Performing intelligent restart with {'memory-based ensemble' if memory_solutions else 'random initialization'}")
    
    return field_values

def create_convergence_plot(errors, perturbation_iterations, restart_iterations, convergence_threshold):
    ...
    try:
        ...
        rate_line, = ax2.semilogy(error_rate_of_change, color=color2, linestyle='--', label='Rate of Change')
        ...
        # Add threshold line
        threshold_line = ax1.axhline(y=convergence_threshold, color='k', linestyle='-.',
...
def compile_statistics(iterations, error, best_error, perturbation_iterations, restart_iterations, 
                      temperature, strategy_success_counts, strategy_attempt_counts, errors, 
                      evaluation_delay=8):
    ...
    if perturbation_iterations:
        ...
        error_before_perturbation_list = [errors[max(0, idx-1)] for idx in perturbation_iterations]
        ...
        error_reduction = [before - after for before, after in zip(error_before_perturbation_list, error_after_perturbation_list)]
        
        if error_reduction:
            stats["avg_error_reduction_after_perturbation"] = np.mean(error_reduction)
            stats["perturbation_effectiveness"] = np.sum([r > 0 for r in error_reduction]) / len(error_reduction)
