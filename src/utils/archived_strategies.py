...
def apply_perturbation_strategy(
    ...
    if strategy == 0:
        ...
        random_coefficients = np.random.normal(0, 1, cluster_coefficients.shape) + 1j * np.random.normal(0, 1, cluster_coefficients.shape)
    ...
    # Strategy 1: CATASTROPHIC phase inversion plus magnitude explosion
        ...
        if verbose:
            logger.info(f"🔄 CATASTROPHIC phase inversion + magnitude explosion at iteration {iteration}")
    
    # Strategy 2: CHAOTIC basin hopping with massive jump
    elif strategy == 2:
        # Create a completely random direction with no relation to current field
        random_direction = np.random.normal(0, 1, field_values.shape) + 1j * np.random.normal(0, 1, field_values.shape)
    ...
    # Strategy 4: Simulated annealing with temperature-based jumps
        ...
        if verbose:
            logger.info(f"Applied simulated annealing jump with temperature {current_temp:.4f} at iteration {iteration}")
    ...
    # Strategy 5: Anti-correlation perturbation (move opposite to current gradient direction)
        ...
        if previous_field_values is not None:
            ...
            magnitude = np.abs(field_values)
            phase = np.angle(field_values)
            field_values = magnitude * np.exp(1j * (phase + np.random.normal(0, np.pi, phase.shape)))
    ...
    return field_values, skip_magnitude_constraint, best_coefficients, best_error, memory_solutions, memory_errors
...
def perform_intelligent_restart(
    ...
    if memory_solutions:
        ...
        if verbose:
            logger.info(f"Intelligent restart: {random_ratio*100:.1f}% random, {memory_ratio*100:.1f}% memory-based")
            logger.info(f"Using {len(memory_solutions)} memory solutions with temperature factor {t_factor:.4f}")
