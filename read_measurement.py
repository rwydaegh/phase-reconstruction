...
def examine_pickle_file(file_path):
    ...
    for key, value in data.items():
        if isinstance(value, np.ndarray):
            ...
            if key == 'results':
                ...
                print(f"  Spacing: {(np.max(value) - np.min(value)) / (value.shape[0] - 1) if value.shape[0] > 1 else 0} mm (average)")
    ...
    if 'results' in data and 'continuous_axis' in data and 'discrete_axis' in data and 'points_continuous' in data and 'points_discrete' in data:
        plot_data(data)
...
def plot_data(data, method='contourf', in_log=False, number_of_levels=None, filename=None):
    ...
    if method == 'contourf':
        contour = plt.contourf(cc, dd, results, levels=number_of_levels, cmap='jet')
    elif method == 'imshow':
        plt.imshow(results, cmap='jet', aspect='auto', 
    ...
    if filename is None:
        filename = 'measurement_visualization_log.png' if in_log else 'measurement_visualization.png'
...
if __name__ == "__main__":
    ...
    plot_data(data, method='contourf', in_log=False, 
              filename='measurement_visualization.png')
    
    # Also plot with log scale
    print("\nCreating log-scale plot...")
    plot_data(data, method='contourf', in_log=True, 
              filename='measurement_visualization_log.png')
    
    # Also try other visualization methods
    print("\nCreating pcolor visualization...")
    plot_data(data, method='pcolor', in_log=False, 
              filename='measurement_visualization_pcolor.png')
