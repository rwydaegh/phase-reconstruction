...
def load_measurement_data(file_path):
    ...
    logger.info(f"Continuous axis ({continuous_axis}): {len(points_continuous)} points from {np.min(points_continuous):.1f} to {np.max(points_continuous):.1f} mm")
    logger.info(f"Discrete axis ({discrete_axis}): {len(points_discrete)} points from {np.min(points_discrete):.1f} to {np.max(points_discrete):.1f} mm")
    logger.info(f"Frequency: {frequency/1e9:.2f} GHz (wavelength: {299792458/(frequency/1e9)/1e6:.2f} mm)")
...
def sample_measurement_data(data, target_resolution=50):
    ...
    assert(target_resolution <= n_discrete, f"Target resolution {target_resolution} must be less than or equal to the original resolution of discrete axis {n_discrete}")
    ...
    logger.info(f"Sampled data dimensions: {len(sampled_points_discrete)}×{len(sampled_points_continuous)}")
...
def physical_to_pixel_radius(R_mm, points_continuous, points_discrete):
    ...
    cont_spacing = (np.max(points_continuous) - np.min(points_continuous)) / (len(points_continuous) - 1)
...
def main():
    ...
    if config.use_source_pointcloud:
        ...
        if isinstance(data, np.ndarray):
            if data.shape[1] >= 3:
                ...
                logger.info(f"Downsampled point cloud from {len(data)} to {len(points)} points (factor: {config.pointcloud_downsample})")
                ...
                if hasattr(config, 'max_distance_from_origin') and config.max_distance_from_origin < float('inf'):
                    ...
                    logger.info(f"Filtered out {orig_num_points - len(points)} points more than {config.max_distance_from_origin}m from origin")
                    logger.info(f"Retained {len(points)} points within {config.max_distance_from_origin}m from origin")
            else:
        ...
        logger.info(f"Created cube environment with {len(points)} points, cube size: {config.room_size}m")
    ...
    if config.use_source_pointcloud and hasattr(config, 'use_measurement_frequency') and config.use_measurement_frequency:
    ...
    if hasattr(config, 'uniform_current_init') and config.uniform_current_init:
        logger.info(f"Initializing currents with uniform amplitude {config.initial_current_amplitude} and random phase")
        ...
        H_pinv_init = compute_pseudoinverse(H, config.regularization, config.adaptive_regularization)
        ...
        logger.info(f"Initialized field values from uniform currents with shape {initial_field_values.shape}")
    ...
    if stats['post_perturbation_tracking']:
        ...
        success_rate = len(successful_perturbations) / len(stats['post_perturbation_tracking']) if stats['post_perturbation_tracking'] else 0
    ...
    if config.enable_smoothing:
        ...
        R_pixels = physical_to_pixel_radius(config.smoothing_radius_mm, 
                                          sampled_data['points_continuous'], 
                                          sampled_data['points_discrete'])
        
        logger.info(f"Converted physical radius {config.smoothing_radius_mm} mm to pixel radius {R_pixels:.2f}")
        ...
        smoothed_reconstructed_field_abs_2d = apply_gaussian_smoothing(reconstructed_field_abs_2d, R_pixels)
        ...
        smoothed_reconstructed_field_2d = smoothed_reconstructed_field_abs_2d * np.exp(1j * reconstructed_field_phase)
    ...
    # Print metrics
    logger.info(f"Reconstruction quality metrics:")
