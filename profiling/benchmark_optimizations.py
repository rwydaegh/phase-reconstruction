...
def benchmark_svd_methods(config: BenchmarkConfig):
    ...
    for resolution in resolutions:
        ...
        if not np.isnan(randomized_total):
            ...
            logger.info(f"  Standard: {standard_avg:.4f}s, Randomized: {randomized_avg:.4f}s, "
...
def visualize_results(
    resolutions, title, y_label, data_sets, labels, colors, styles, 
    filename, y_log=False, speedup=True
):
...
def run_all_benchmarks():
    ...
    resolutions, original_times, optimized_times, optimized_randomized_times = benchmark_holographic_phase_retrieval(config)
    visualize_results(
        resolutions,
...
def generate_summary(
    resolutions, original_channel_times, optimized_channel_times,
    standard_svd_times, randomized_svd_times,
    original_hpr_times, optimized_hpr_times, optimized_randomized_hpr_times
):
    """Generate a summary of benchmark results."""
    # Calculate average speedups
    channel_speedups = [o/n for o, n in zip(original_channel_times, optimized_channel_times) 
                        if not np.isnan(o) and not np.isnan(n)]
    svd_speedups = [o/n for o, n in zip(standard_svd_times, randomized_svd_times) 
                   if not np.isnan(o) and not np.isnan(n)]
    hpr_speedups = [o/n for o, n in zip(original_hpr_times, optimized_hpr_times) 
                   if not np.isnan(o) and not np.isnan(n)]
    hpr_rand_speedups = [o/n for o, n in zip(original_hpr_times, optimized_randomized_hpr_times) 
                        if not np.isnan(o) and not np.isnan(n)]
    ...
    with open("optimization_summary.txt", "w") as f:
        ...
        f.write(f"Phase Retrieval (Optimized + Randomized SVD): {avg_hpr_rand_speedup:.2f}x speedup\n\n")
        
        f.write("Detailed Results by Resolution:\n")
        f.write("-------------------------------\n")
        for i, res in enumerate(resolutions):
            f.write(f"\nResolution: {res}x{res}\n")
            f.write(f"  Channel Matrix: {original_channel_times[i]:.4f}s -> {optimized_channel_times[i]:.4f}s ")
            ...
            f.write(f"  Phase Retrieval: {original_hpr_times[i]:.4f}s -> {optimized_hpr_times[i]:.4f}s ")
            ...
            f.write(f"  Phase Retrieval + Randomized SVD: {original_hpr_times[i]:.4f}s -> {optimized_randomized_hpr_times[i]:.4f}s ")
            if not np.isnan(original_hpr_times[i]) and not np.isnan(optimized_randomized_hpr_times[i]):
