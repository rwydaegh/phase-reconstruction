...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Run sensitivity analysis for field reconstruction")
    ...
    parser.add_argument('--gs-iterations', type=int, default=200,
                        help='Maximum Gerchberg-Saxton iterations (default: 200)')
    parser.add_argument('--perturb-points', action='store_true',
                        help='Enable point cloud perturbation')
    parser.add_argument('--perturbation-factor', type=float, default=0.01,
...
def create_rmse_plot(X, Y, rmse, param1_name, param2_name, save_dir):
    ...
    im = ax.pcolormesh(X, Y, rmse_viz, cmap=cmap, 
                      vmin=np.nanmin(rmse), vmax=normal_max)
        ...
        for iy in range(X.shape[1]):
            if is_outlier[ix, iy]:
                ...
                rect = patches.Rectangle(
                    ...
                    X[0, 1] - X[0, 0], Y[1, 0] - Y[0, 0],
                    linewidth=2, edgecolor='red', facecolor='none'
                )
                ax.add_patch(rect)
                
                # Add text with the actual value
                ax.text(X[ix, iy], Y[ix, iy], f"{rmse[ix, iy]:.3f}", 
    ...
    if outlier_count > 0:
        ax.text(0.5, 1.05, f"⚠️ {outlier_count} outlier(s) found - marked in red",
    ...
    for ix in range(X.shape[0]):
        for iy in range(X.shape[1]):
            ...
            if correlation[ix, iy] < 0.999:
                ax.text(X[ix, iy], Y[ix, iy], f"{correlation[ix, iy]:.3f}", 
                       ha='center', va='center', color='black',
