...
def test_load_real_plane_basic(mock_sample_data, mock_load_data, dummy_pickle_file):
    ...
    mock_load_data.return_value = {
        ...
        "continuous_axis": "z",
        "discrete_axis": "y",
        "points_continuous": np.linspace(-1, 1, 100),
        "points_discrete": np.linspace(-1, 1, 100),
        "frequency": 28e9,
    }
    mock_sample_data.return_value = {
        "results": np.random.rand(target_res, target_res),
        "continuous_axis": "z",
        "discrete_axis": "y",
        "points_continuous": np.linspace(-1, 1, target_res),
        "points_discrete": np.linspace(-1, 1, target_res),
        "frequency": 28e9,
    }
    ...
    plane_cfg = OmegaConf.create(
        {
            ...
            "measured_data_path": os.path.basename(
                dummy_pickle_file
...
def test_load_real_plane_file_not_found():
    ...
    plane_cfg = OmegaConf.create(
        {"measured_data_path": "non_existent_file.pkl", "target_resolution": 50}
    )
...
@pytest.mark.parametrize("plane_type, expected_fixed_axis_index", [("xy", 2), ("yz", 0), ("xz", 1)])
...
def test_process_plane_definitions(mock_gen_sim, mock_load_real):
    ...
    mock_load_real.side_effect = [
        {
            ...
            "measured_magnitude": np.random.rand(100),
        },  # Return value for 'real1' call
        {
            "coordinates": np.random.rand(64, 3),
            "measured_magnitude": np.random.rand(64),
        },  # Return value for 'real2' call
    ]
    mock_gen_sim.return_value = {
        "coordinates": np.random.rand(25, 3)
    }  # Only called once, return_value is fine
    ...
    assert (
        mock_load_real.call_args_list[0][0][0] is plane_configs[0]
    )  # First call with first config
    assert (
        mock_gen_sim.call_args_list[0][0][0] is plane_configs[1]
    )  # Second call (first sim) with second config
    assert (
        mock_load_real.call_args_list[1][0][0] is plane_configs[2]
    )  # Third call (second real) with third config
        ...
        OmegaConf.create(
            {
                "name": "fail_real",
                "is_real_plane": True,
                "measured_data_path": "fail",
                "target_resolution": 10,
            }
        ),
        OmegaConf.create(
            {"name": "sim_ok", "is_real_plane": False, "plane_type": "xy", "resolution": 5}
        ),
    ]
