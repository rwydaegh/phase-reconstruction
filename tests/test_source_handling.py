...
def dummy_pointcloud_file_factory():
    temp_dirs = []
    def _create_file(filename="dummy_points.pkl", shape=(50, 3), add_tangents=False, add_normals=False):
...
def test_load_from_file_basic_3col(dummy_pointcloud_file_factory):
    ...
    pts_t, t1_t, t2_t, pts_p, t1_p, t2_p, _ = source_handling.get_source_pointcloud(cfg, dummy_global_cfg, base_dir=tmpdir) # Add global_cfg and unpack currents
...
def test_load_from_file_13col(dummy_pointcloud_file_factory):
    ...
    dummy_global_cfg = OmegaConf.create({'use_vector_model': True}) # Use vector model true since tangents are expected
    pts_t, t1_t, t2_t, pts_p, t1_p, t2_p, _ = source_handling.get_source_pointcloud(cfg, dummy_global_cfg, base_dir=tmpdir) # Add global_cfg and unpack currents
...
def test_load_file_not_found():
    ...
    cfg = OmegaConf.create({
        'use_source_file': True,
        'source_file_path': 'non_existent.pkl'
    })
...
def test_generate_cloud(mock_get_tangents, mock_get_normals, mock_create_cloud):
    ...
    mock_get_normals.return_value = (np.random.rand(60, 3), np.random.rand(60, 3)) # points, normals
    ...
    dummy_global_cfg = OmegaConf.create({'use_vector_model': True}) # Assume vector model for generated tangents
    pts_t, t1_t, t2_t, pts_p, t1_p, t2_p, _ = source_handling.get_source_pointcloud(cfg, dummy_global_cfg, base_dir=".") # Add global_cfg and unpack currents
...
def test_max_distance_filter(dummy_pointcloud_file_factory):
    ...
    with open(filepath, 'wb') as f: pickle.dump(points, f)
    ...
    dummy_global_cfg = OmegaConf.create({'use_vector_model': False})
    pts_t, _, _, pts_p, _, _, _ = source_handling.get_source_pointcloud(cfg, dummy_global_cfg, base_dir=tmpdir) # Add global_cfg and unpack currents
