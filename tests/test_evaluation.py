...
def vector_global_config():
    """Vector model config."""
    return OmegaConf.create(
        {"wavelength": 0.01, "use_vector_model": True, "measurement_direction": [1.0, 0.0, 0.0]}
    )
...
def dummy_geometry():
    ...
    t1 = np.random.rand(n_points, 3)
    t1 /= np.linalg.norm(t1, axis=1)[:, np.newaxis]
    t2 = np.random.rand(n_points, 3)
    ...
    return {
        ...
        "tangents1_perturbed": t1.copy(),
        "tangents2_perturbed": t2.copy(),
    }
...
def test_evaluation_scalar(
    mock_create_H,
    default_global_config,
    dummy_geometry,
    dummy_coeffs_scalar,
    dummy_currents_scalar,
    dummy_test_planes,
):
    ...
    mock_H_true_sim = np.random.rand(n_meas2, n_points) + 1j * np.random.rand(
        n_meas2, n_points
    )  # For sim_test ground truth (n_meas2 points)
    mock_H_test_real = np.random.rand(n_meas1, n_points) + 1j * np.random.rand(
        n_meas1, n_points
    )  # For real_test reconstruction (n_meas1 points)
    mock_H_test_sim = np.random.rand(n_meas2, n_points) + 1j * np.random.rand(
        n_meas2, n_points
    )  # For sim_test reconstruction (n_meas2 points)
    ...
    assert np.allclose(
        mock_create_H.call_args_list[0][1]["points"], dummy_geometry["points_perturbed"]
    )  # Call 1: H_test_real
    assert np.allclose(
        mock_create_H.call_args_list[1][1]["points"], dummy_geometry["points_true"]
    )  # Call 2: H_true_sim
    assert np.allclose(
        mock_create_H.call_args_list[2][1]["points"], dummy_geometry["points_perturbed"]
    )  # Call 3: H_test_sim
    ...
    assert results["real_test"]["ground_truth_magnitude"].shape == (
        n_meas1,
    )  # From dummy_test_planes
    assert results["real_test"]["reconstructed_magnitude"].shape == (
        n_meas1,
    )  # H_test_real is (n_meas1, n_points), coeffs is (n_points,)
    assert results["sim_test"]["ground_truth_magnitude"].shape == (
        n_meas2,
    )  # H_true_sim is (n_meas2, n_points), currents is (n_points,)
    assert results["sim_test"]["reconstructed_magnitude"].shape == (
        n_meas2,
    )  # H_test_sim is (n_meas2, n_points), coeffs is (n_points,)


@patch("src.evaluation.create_channel_matrix")
def test_evaluation_vector(
    mock_create_H,
    vector_global_config,
    dummy_geometry,
    dummy_coeffs_vector,
    dummy_currents_vector,
    dummy_test_planes,
):
    ...
    mock_H_true_sim = np.random.rand(n_meas2, 2 * n_points) + 1j * np.random.rand(
        n_meas2, 2 * n_points
    )  # For sim_test ground truth (n_meas2 points)
    mock_H_test_real = np.random.rand(n_meas1, 2 * n_points) + 1j * np.random.rand(
        n_meas1, 2 * n_points
    )  # For real_test reconstruction (n_meas1 points)
    mock_H_test_sim = np.random.rand(n_meas2, 2 * n_points) + 1j * np.random.rand(
        n_meas2, 2 * n_points
    )  # For sim_test reconstruction (n_meas2 points)
    ...
    assert np.allclose(
        mock_create_H.call_args_list[0][1]["tangents1"], dummy_geometry["tangents1_perturbed"]
    )  # Call 1: H_test_real
    assert np.allclose(
        mock_create_H.call_args_list[1][1]["tangents1"], dummy_geometry["tangents1_true"]
    )  # Call 2: H_true_sim
    assert np.allclose(
        mock_create_H.call_args_list[2][1]["tangents1"], dummy_geometry["tangents1_perturbed"]
    )  # Call 3: H_test_sim
...
def test_evaluation_no_test_planes(
    default_global_config, dummy_geometry, dummy_coeffs_scalar, dummy_currents_scalar
):
...
def test_evaluation_h_creation_error(
    mock_create_H,
    default_global_config,
    dummy_geometry,
    dummy_coeffs_scalar,
...
def test_evaluation_coeff_shape_mismatch(
    default_global_config, dummy_geometry, dummy_currents_scalar, dummy_test_planes
):
