...
def test_create_pointcloud_no_interior_points(basic_config: DictConfig):
    ...
    is_interior = (
        (np.abs(points[:, 0]) < half_size - 1e-9)
...
def channel_matrix_setup(basic_config: DictConfig):
    ...
    source_points = np.array([[0.1, 0.2, 0.3], [-0.1, -0.2, -0.3]])
...
def test_create_channel_matrix_shape(channel_matrix_setup):
    ...
    source_points, tangents1, tangents2, measurement_plane, measurement_direction, k = (
        channel_matrix_setup
    )
